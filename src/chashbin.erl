...
-record(chashbin,
        {size :: pos_integer(), owners :: owners_bin(), nodes :: erlang:tuple(node())}).
...
-record(iterator,
        {pos :: non_neg_integer(), start :: non_neg_integer(), chbin :: chashbin()}).
...
create({Size, Owners}) ->
    ...
    #chashbin{size = Size, owners = Bin, nodes = list_to_tuple(Nodes2)}.
...
to_list(#chashbin{owners = Bin, nodes = Nodes}) ->
    [{Idx, element(Id, Nodes)} || <<Idx:160/integer, Id:16/integer>> <= Bin].
...
to_list_filter(Pred, #chashbin{owners = Bin, nodes = Nodes}) ->
    [{Idx, element(Id, Nodes)}
     || <<Idx:160/integer, Id:16/integer>> <= Bin, Pred({Idx, element(Id, Nodes)})].
...
itr_pop(N, Itr = #iterator{pos = Pos, chbin = CHBin}) ->
    ...
    L = case Bin of
          <<_:Pos/binary-unit:176, Bin2:N/binary-unit:176, _/binary>> ->
              [{Idx, element(Id, Nodes)} || <<Idx:160/integer, Id:16/integer>> <= Bin2];
          _ ->
              Left = N + Pos - Size,
              Skip = Pos - Left,
              <<Bin3:Left/binary-unit:176, _:Skip/binary-unit:176, Bin2/binary>> = Bin,
              L1 = [{Idx, element(Id, Nodes)} || <<Idx:160/integer, Id:16/integer>> <= Bin2],
              L2 = [{Idx, element(Id, Nodes)} || <<Idx:160/integer, Id:16/integer>> <= Bin3],
              L1 ++ L2
        end,
    Pos2 = (Pos + N) rem Size,
