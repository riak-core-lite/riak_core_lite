...
get_apl_chbin(DocIdx, N, CHBin, UpNodes) ->
    [{Partition, Node}
     || {{Partition, Node}, _Type} <- get_apl_ann_chbin(DocIdx, N, CHBin, UpNodes)].
...
get_apl(DocIdx, N, Ring, UpNodes) ->
    [{Partition, Node}
     || {{Partition, Node}, _Type} <- get_apl_ann(DocIdx, N, Ring, UpNodes)].
...
-spec check_up(preflist(), [node()], preflist_ann(), preflist()) -> {preflist_ann(),
                                                                     preflist()}.
...
find_fallbacks([{Partition, _Node} | Rest] = Pangs,
    ...
    case is_up(FN, UpNodes) of
      true ->
          find_fallbacks(Rest, Fallbacks, UpNodes, [{{Partition, FN}, fallback} | Secondaries]);
      false ->
          find_fallbacks(Pangs, Fallbacks, UpNodes, Secondaries)
    end.

%% @doc Find fallbacks for downed nodes in the preference list.
-spec find_fallbacks_chbin(preflist(),
                           iterator(),
                           [node()],
...
find_fallbacks_chbin([{Partition, _Node} | Rest] = Pangs, Itr, UpNodes, Secondaries) ->
    ...
    case is_up(FN, UpNodes) of
      true ->
          find_fallbacks_chbin(Rest, Itr2, UpNodes, [{{Partition, FN}, fallback} | Secondaries]);
      false ->
...
perfect_ring(RingSize, Nodes) when RingSize rem length(Nodes) =:= 0 ->
    ...
    TransferNode = fun ({Idx, _CurOwner}, {Ring0, [NewOwner | Rest]}) ->
...
six_node_test() ->
    ...
    ?assertEqual([{433883298582611803841718934712646521460354973696, 'dev5@127.0.0.1'},
                 ...
                 get_apl(DocIdx,
                         3,
                         Ring,
                         Nodes -- ['dev2@127.0.0.1', 'dev3@127.0.0.1', 'dev4@127.0.0.1'])),
    ...
    ?assertEqual([{433883298582611803841718934712646521460354973696, 'dev2@127.0.0.1'},
                 ...
                 get_apl(DocIdx,
                         3,
                         Ring,
                         Nodes -- ['dev4@127.0.0.1', 'dev3@127.0.0.1', 'dev6@127.0.0.1'])),
...
six_node_bucket_key_ann_test() ->
    ...
    application:set_env(riak_core,
                        ...
                        [{n_val, 3}, {chash_keyfun, {riak_core_util, chash_std_keyfun}}]),
...
chbin_test_scenario() ->
    [chbin_test_scenario(Size, NumNodes)
     || Size <- [32, 64, 128], NumNodes <- [1, 2, 3, 4, 5, 8, Size div 4]],
    ok.
...
chbin_test_scenario(Size, NumNodes) ->
    ...
    [begin
     ...
     || HashKey <- HashKeys, N <- [1, 2, 3, 4], Down <- [0, 1, 2, Size div 2, Size - 1, Size]],
    ok.
