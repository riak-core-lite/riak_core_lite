...
init([Parent, RegName, Mod, Index]) ->
    ...
    Interval = application:get_env(riak_core, vnode_check_interval, ?DEFAULT_CHECK_INTERVAL),
    ...
    SafeInterval = case Threshold == undefined orelse Interval < Threshold of
                     true ->
                         Interval;
                     false ->
    ...
    SafeRequestInterval = case RequestInterval < SafeInterval of
                            true ->
                                RequestInterval;
                            false ->
                                logger:warning("Setting riak_core/vnode_check_request_interval to ~b",
                                               [SafeInterval div 2]),
                                SafeInterval div 2
                          end,
...
-spec command_return_vnode({atom(), non_neg_integer(), atom()}, term()) -> {ok, pid()} |
                                                                           {error, term()}.
...
handle_call(overloaded,
            _From,
            State = #state{check_mailbox = Mailbox, check_threshold = Threshold}) ->
...
handle_cast({vnode_proxy_pong, Ref, Msgs},
            State = #state{check_request = RequestState, check_mailbox = Mailbox}) ->
       ...
       fun ({VnodePid, ProxyPid}) ->
               {"should tolerate slow vnodes",
                ...
                fun () ->
                        ...
                        %% reasonable
                        {message_queue_len, L} = erlang:process_info(VnodePid, message_queue_len),
