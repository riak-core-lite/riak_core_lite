...
-record(xfer_status,
        {status :: pending | complete, mod_src_target :: {module(), index(), index()}}).
...
-spec repair(atom(), {module(), partition()}, {module(), atom()}) -> {ok,
                                                                      Pairs :: [{partition(),
                                                                                 node()}]} |
                                                                     {down,
                                                                      Down :: [{partition(),
...
get_all_index_pid(Mod, Default) ->
    try
      [list_to_tuple(L) || L <- ets:match(?ETS, {idxrec, '_', '$1', Mod, '$2', '_'})]
    catch
      _:_ ->
          Default
    end.
...
find_vnodes(State) ->
    ...
    %% vnode.
    VnodePids = [Pid
                 || {_, Pid, worker, _} <- supervisor:which_children(riak_core_vnode_sup),
    ...
    F = fun (Pid, Idx, Mod) ->
                ...
                #idxrec{key = {Idx, Mod}, idx = Idx, mod = Mod, pid = Pid, monref = Mref}
        end,
...
handle_call({xfer_complete, ModSrcTgt}, _From, State) ->
    ...
    case get_repair(ModPartition, Repairs) of
      none ->
          logger:error("Received xfer_complete for non-existing repair: ~p", [ModPartition]),
          {reply, ok, State};
      #repair{minus_one_xfer = MOX, plus_one_xfer = POX} = R ->
          R2 = if ?XFER_EQ(MOX, ModSrcTgt) ->
                  ...
                  true ->
                      logger:error("Received xfer_complete for non-existing xfer: ~p", [ModSrcTgt])
               end,

          case {?XFER_COMPLETE(R2#repair.minus_one_xfer), ?XFER_COMPLETE(R2#repair.plus_one_xfer)}
              of
            {true, true} ->
    ...
    lists:flatten(lists:map(fun (#repair{pairs =
                                         ...
                                         minus_one_xfer =
                                             #xfer_status{mod_src_target =
                                                              {M1Mod, M1SrcIdx, _M1DstIdx}},
                                         plus_one_xfer =
                                             #xfer_status{mod_src_target =
                                                              {P1Mod, P1SrcIdx, _P1DstIdx}}}) ->
...
create_repair(Pairs, ModPartition, FilterModFun, Mod, Partition, Repairs, State) ->
    ...
    riak_core_handoff_manager:xfer(MinusOne, ModPartition, FilterModFun),
    riak_core_handoff_manager:xfer(PlusOne, ModPartition, FilterModFun),
    MOXStatus = #xfer_status{status = pending, mod_src_target = {Mod, MOP, Partition}},
    POXStatus = #xfer_status{status = pending, mod_src_target = {Mod, POP, Partition}},
...
handle_cast(force_handoffs, State) ->
    ...
    _ = [maybe_trigger_handoff(Mod, Idx, Pid, State2) || {Mod, Idx, Pid} <- AllVNodes],
...
schedule_management_timer() ->
    ManagementTick = application:get_env(riak_core, vnode_management_timer, 10000),
...
limit_ownership_handoff(Limit, Transfers, true) ->
    ...
    Filtered = [Transfer
                || {_, _, _, _, Status} = Transfer <- Transfers, Status =:= awaiting],
...
get_vnode(IdxList, Mod, State) ->
    ...
    Pairs = Started ++
              riak_core_util:pmap(StartFun, NotStarted, ?DEFAULT_VNODE_ROLLING_START),
    %% Return Pids in same order as input
    PairsDict = dict:from_list(Pairs),
    _ = [begin
           Pid = dict:fetch(Idx, PairsDict),
           MonRef = erlang:monitor(process, Pid),
           IdxRec = #idxrec{key = {Idx, Mod}, idx = Idx, mod = Mod, pid = Pid, monref = MonRef},
...
compute_forwarding(Mods, Ring) ->
    ...
    Forwarding = [check_forward_precomputed(Completed, Mod, I, N, Ring)
                  || {I, N} <- riak_core_ring:all_owners(Ring), Mod <- Mods],
    dict:from_list(Forwarding).

update_forwarding(AllVNodes, Mods, Ring, State = #state{forwarding = Forwarding}) ->
...
should_handoff(Ring, _CHBin, Mod, Idx) ->
    ...
    case determine_handoff_target(Type, NextOwner, Ready, IsResizing) of
      undefined ->
          false;
      Action when Action =:= '$resize' orelse Action =:= '$delete' ->
          {true, Action};
      TargetNode ->
...
get_all_vnodes_status(#state{forwarding = Forwarding, handoff = HO}) ->
    ...
    Forwarding2 = [{MI, {forwarding, Node}} || {MI, Node} <- Forwarding1, Node /= undefined],
    Handoff1 = lists:sort(dict:to_list(HO)),
    Handoff2 = [{MI, {should_handoff, Node}} || {MI, {_Type, _Direction, Node}} <- Handoff1],
...
update_never_started(Ring, State = #state{known_modules = KnownMods}) ->
    UnknownMods = [Mod
                   || {_App, Mod} <- riak_core:vnode_modules(), not lists:member(Mod, KnownMods)],
    case UnknownMods of
      [] ->
          State;
      _ ->
...
maybe_start_vnodes(State = #state{vnode_start_tokens = Tokens,
    ...
    case {Tokens, NeverStarted} of
      ...
      {_, [{Idx, Mod} | NeverStarted2]} ->
          ...
          State#state{vnode_start_tokens = Tokens - 1, never_started = NeverStarted2}
    end.

-spec check_repairs(repairs()) -> Repairs2 :: repairs().
check_repairs(Repairs) ->
    Check = fun (R = #repair{minus_one_xfer = MOX, plus_one_xfer = POX}, Repairs2) ->
...
maybe_retry(R, {SrcPartition, _} = Src, Xfer) ->
    case Xfer#xfer_status.status of
      ...
      pending ->
          ...
          #xfer_status{status = pending, mod_src_target = {Mod, SrcPartition, Partition}}
    end.

%% @private
%%
%% @doc Verify that all nodes are up involved in the repair.
-spec check_up([{non_neg_integer(), node()}], [node()]) -> true |
                                                           {false,
                                                            Down :: [{non_neg_integer(), node()}]}.
check_up(Pairs, UpNodes) ->
    Down = [Pair || {_Partition, Owner} = Pair <- Pairs, not lists:member(Owner, UpNodes)],
    case Down of
      [] ->
          true;
      _ ->
...
-spec repair_pairs(riak_core_ring:riak_core_ring(), non_neg_integer()) -> [{Partition ::
                                                                                non_neg_integer(),
                                                                            Owner :: node()}].
...
replace_repair(Repair, Repairs) ->
    lists:keyreplace(Repair#repair.mod_partition, #repair.mod_partition, Repairs, Repair).
...
kill_repair(Repair, Reason) ->
    ...
    riak_core_handoff_manager:kill_xfer(node(), {Mod, undefined, Partition}, Reason).
