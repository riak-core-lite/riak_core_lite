...
choose_claim_v2(Ring, Node, Params0) ->
    ...
    AllIndices = lists:zip(lists:seq(0, length(Owners) - 1), [Idx || {Idx, _} <- Owners]),
    ...
    case EnoughNodes of
      ...
      false ->
          ...
          AllNth = lists:sublist([(X + Y) rem RingSize || Y <- Offsets, X <- PreferredNth],
                                 RingSize),
          Indices = [lists:keyfind(Nth, 1, AllIndices) || Nth <- AllNth]
    end,

    %% Filter out indices that conflict with the node's existing ownership
    Indices2 = prefilter_violations(Ring, Node, AllIndices, Indices, TargetN, RingSize),
...
-spec get_deltas(RingSize :: pos_integer(),
                 ...
                 Counts :: [{node(), non_neg_integer()}]) -> Deltas :: [{node(), integer()}].
...
-spec rebalance_deltas([{node(), integer()}], pos_integer(), pos_integer()) -> [{node(),
                                                                                 integer()}].
...
-spec increase_keeps(Deltas :: [{node(), integer()}],
                     WantsError :: integer(),
                     Max :: pos_integer(),
                     Acc :: [{node(), integer()}]) -> Rebalanced :: [{node(), integer()}].
    ...
    case lists:keytake(Node, 1, Last) of
      {value, {Node, LastIndex, _}, NewLast} ->
          if Index - LastIndex >= TargetN ->
                 ...
                 meets_target_n(Rest, TargetN, Index + 1, First, [{Node, Index, Part} | NewLast]);
             true ->
...
claim_diversify(Wants, Owners, Params) ->
    ...
    {ok, NewOwners, _AM} =
        riak_core_claim_util:construct(riak_core_claim_util:gen_complete_len(Q), Claiming, TN),
...
-spec sequential_claim(riak_core_ring:riak_core_ring(),
                       node(),
                       integer()) -> riak_core_ring:riak_core_ring().
sequential_claim(Ring, Node, TargetN) ->
...
build_nodelist(RingSize, Nodes, _Shortfall = 0, _NodeCounter, _MinFetchesPerSeq, Acc) ->
    ...
    backfill_ring(RingSize, Nodes, (RingSize - length(ShuffledRing)) div length(Nodes), Acc);
...
random_choose_claim(Ring, Node, _Params) ->
    riak_core_ring:transfer_node(riak_core_ring:random_other_index(Ring), Node, Ring).
...
-spec get_counts([node()], [{integer(), _}]) -> [{node(), non_neg_integer()}].
...
select_indices(Owners, Deltas, Indices, TargetN, RingSize) ->
    ...
    {Claim, _, _, _} = lists:foldl(fun ({Nth, Idx}, {Out, LastNth, DeltaDT, First}) ->
                                           ...
                                           case (Delta < 0) and (First or MeetsTN) of
                                             true ->
                                                 NextDeltaDT = dict:update_counter(Owner,
                                                                                   1,
                                                                                   DeltaDT),
...
prop_claim_ensures_unique_nodes(ChooseFun) ->
    ...
    ?FORALL({PartsPow, NodeCount},
            ...
            begin
              ...
              ImperfectPLs = orddict:to_list(lists:foldl(fun (PL, Acc) ->
              ...
              ?WHENFAIL(begin
                          ...
                          io:format(user, "Owners: ~p~n", [riak_core_ring:all_owners(Rfinal)])
                        end,
                        conjunction([{meets_target_n,
                                      equals({true, []}, meets_target_n(Rfinal, TNval))},
                                     {perfect_preflists, equals([], ImperfectPLs)},
                                     {balanced_ring,
                                      balanced_ring(Partitions, NodeCount, Rfinal)}]))
            end).
...
balanced_ring(RingSize, NodeCount, Ring) ->
    ...
    {BalancedMax, AccFinal} = lists:foldl(fun ({_Part, Node},
...
prop_wants_counts() ->
    ?FORALL({S, Q},
            {large_pos(100), large_pos(100000)},
            begin
              Wants = wants_counts(S, Q),
              conjunction([{len, equals(S, length(Wants))}, {sum, equals(Q, lists:sum(Wants))}])
            end).
...
prop_wants() ->
    ?FORALL({NodeStatus, Q},
            {?SUCHTHAT(L, non_empty(list(elements([leaving, joining]))), lists:member(joining, L)),
            ...
            begin
              ...
              {_, R2, Active} = lists:foldl(fun (S, {I, R1, A1}) ->
                                                    ...
                                                    case S of
                                                      joining ->
                                                          {I + 1,
                                                           riak_core_ring:add_member(N, R1, N),
                                                           [N | A1]};
                                                      _ ->
                                                          {I + 1,
                                                           riak_core_ring:leave_member(N, R1, N),
                                                           A1}
                                                    end
                                            end,
                                            {1, R0, []},
              ...
              {ActiveWants, InactiveWants} = lists:partition(fun ({N, _W}) ->
                                                                     lists:member(N, Active)
                                                             end,
                                                             Wants),
