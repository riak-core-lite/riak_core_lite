...
ring_stats(R, TN) ->
    violation_stats(R, TN) ++ balance_stats(R) ++ diversity_stats(R, TN).
...
node_load(R, NVal, DownNodes) ->
    ...
    [#load{node = N, num_pri = NumPris, num_fb = NumFBs, norm_fb = norm_fb(NumFBs, TotFBs)}
     || {N, NumPris, NumFBs} <- VL].

vnode_load(R, NVal, DownNodes) ->
    UpNodes = riak_core_ring:all_members(R) -- DownNodes,
    Keys = [<<(I + 1):160/integer>> || {I, _Owner} <- riak_core_ring:all_owners(R)],
...
analyze_load(Down, Load) ->
    FBStats = lists:foldl(fun (#load{num_fb = NumFB}, Acc) ->
                          ...
                          basho_stats_histogram:new(1, 1024, 1024),
                          Load),
...
score_am([], _NVal) ->
    undefined;
score_am(AM, NVal) ->
    Cs = lists:flatten([begin [C || {D, C} <- count(Ds, NVal), D < NVal] end
                        || {_Pair, Ds} <- AM]),
    rms(Cs).
...
gen_complete_diverse(RequiredDs) ->
    fun (Owners, DAM) ->
            ...
            {met_required(Owners, DAM, RequiredDs) andalso OwnersLen == NextPow2, NextPow2}
    end.
...
met_required(Owners, DAM, RequiredDs) ->
    ...
    case [Pair || {Pair, Ds} <- dict:to_list(FixupDAM), RequiredDs -- Ds /= []] of
      [] ->
          true;
      _ ->
          false
...
prepend_next_owner(M, Eligible, Owners, DAM, TN) ->
    {_BestScore, Owners2, DAM2} = lists:foldl(fun (Node,
                                                   {RunningScore, _RunningO, _RunningDAM} = Acc) ->
                                                      {Owners1, DAM1} = prepend(M,
                                                                                Node,
                                                                                Owners,
                                                                                DAM),
                                                      case score_am(dict:to_list(DAM1), TN) of
                                                        BetterScore
                                                            when BetterScore < RunningScore ->
...
perm_gen([E]) ->
    [[E]];
perm_gen(L) ->
    lists:append([begin [[X | Y] || Y <- perm_gen(lists:delete(X, L))] end || X <- L]).
