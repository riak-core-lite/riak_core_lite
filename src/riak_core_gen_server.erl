...
-export([start/3,
         ...
         wake_hib/7]).
%% System exports
-export([system_continue/3, system_terminate/4, system_code_change/4, format_status/2]).
%% Internal exports
...
-callback handle_call(Request :: term(),
                      From :: {pid(), Tag :: term()},
                      State :: term()) -> {reply, Reply :: term(), NewState :: term()} |
                                          {reply,
                                           Reply :: term(),
                                           NewState :: term(),
                                           timeout() | hibernate} |
...
-callback handle_cast(Request :: term(), State :: term()) -> {noreply,
                                                              NewState :: term()} |
                                                             {noreply,
                                                              NewState :: term(),
                                                              timeout() | hibernate} |
                                                             {stop,
                                                              Reason :: term(),
                                                              NewState :: term()}.
-callback handle_info(Info :: timeout | term(), State :: term()) -> {noreply,
                                                                     NewState :: term()} |
                                                                    {noreply,
                                                                     NewState :: term(),
                                                                     timeout() | hibernate} |
...
-callback code_change(OldVsn :: term() | {down, term()},
                      State :: term(),
                      Extra :: term()) -> {ok, NewState :: term()} | {error, Reason :: term()}.
...
loop(Parent, Name, State, Mod, hibernate, {Current, Min, undefined}, Queue, Debug) ->
    proc_lib:hibernate(?MODULE,
                       wake_hib,
                       [Parent, Name, State, Mod, {Current, Min, os:timestamp()}, Queue, Debug]);
loop(Parent, Name, State, Mod, Time, TimeoutState, Queue, Debug) ->
    receive
      Input ->
          loop(Parent, Name, State, Mod, Time, TimeoutState, in(Input, Queue), Debug)
      after 0 ->
...
process_next_msg(Parent, Name, State, Mod, Time, TimeoutState, Queue, Debug, Hib) ->
    case riak_core_priority_queue:out(Queue) of
      {{value, Msg}, Queue1} ->
          process_msg(Parent, Name, State, Mod, Time, TimeoutState, Queue1, Debug, Hib, Msg);
      {empty, Queue1} ->
          Time1 = case {Time, TimeoutState} of
                    {binary, {Current, _Min, undefined}} ->
                        Current;
                    _ ->
                        Time
                  end,
          receive
            Input ->
                loop(Parent, Name, State, Mod, Time, TimeoutState, in(Input, Queue1), Debug)
            after Time1 ->
                      process_msg(Parent,
                                  Name,
                                  State,
...
handle_msg({'$gen_call', From, Msg}, Parent, Name, State, Mod, TimeoutState, Queue) ->
    case catch Mod:handle_call(Msg, From, State) of
      ...
      {stop, Reason, Reply, NState} ->
          {'EXIT', R} = (catch terminate(Reason, Name, Msg, Mod, NState, [])),
          reply(From, Reply),
          exit(R);
      Other ->
          handle_common_reply(Other, Parent, Name, Msg, Mod, State, TimeoutState, Queue)
    end;
handle_msg(Msg, Parent, Name, State, Mod, TimeoutState, Queue) ->
...
handle_msg({'$gen_call', From, Msg},
    ...
    case catch Mod:handle_call(Msg, From, State) of
      ...
      {noreply, NState, Time1} ->
          ...
          loop(Parent, Name, NState, Mod, Time1, TimeoutState, Queue, Debug1);
      {stop, Reason, Reply, NState} ->
          {'EXIT', R} = (catch terminate(Reason, Name, Msg, Mod, NState, Debug)),
          _ = reply(Name, From, Reply, NState, Debug),
          exit(R);
      Other ->
          handle_common_reply(Other, Parent, Name, Msg, Mod, State, TimeoutState, Queue, Debug)
    end;
handle_msg(Msg, Parent, Name, State, Mod, TimeoutState, Queue, Debug) ->
    Reply = (catch dispatch(Msg, Mod, State)),
...
system_code_change([Name, State, Mod, Time, TimeoutState, Queue],
                   _Module,
                   OldVsn,
                   Extra) ->
...
error_info(Reason, Name, Msg, State, Debug) ->
    ...
    format("** Generic server ~p terminating \n** Last message in was ~p~n** "
           "When Server state == ~p~n** Reason for termination == ~n** "
           "~p~n",
...
dbg_opts(Name, Opts) ->
    case catch sys:debug_options(Opts) of
      {'EXIT', _} ->
          format("~p: ignoring erroneous debug options - ~p~n", [Name, Opts]),
          [];
      Dbg ->
          Dbg
    end.
...
format_status(Opt, StatusData) ->
    ...
    Specfic = case erlang:function_exported(Mod, format_status, 2) of
                true ->
