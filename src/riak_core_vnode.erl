...
-compile({pulse_replace_module,
          [{gen_fsm_compat, pulse_gen_fsm}, {gen_server, pulse_gen_server}]}).

-endif.

-define(NORMAL_REASON(R),
        R == normal orelse R == shutdown orelse is_tuple(R) andalso element(1, R) == shutdown).
...
-callback init([partition()]) -> {ok, ModState :: term()} |
...
-callback handle_coverage(Request :: term(),
                          ...
                          ModState :: term()) -> continue |
                                                 ...
                                                 {async,
                                                  ...
                                                  NewModState :: term()} |
                                                 {stop, Reason :: term(), NewModState :: term()}.
-callback handle_exit(pid(), Reason :: term(), ModState :: term()) -> {noreply,
                                                                       NewModState :: term()} |
                                                                      {stop,
                                                                       Reason :: term(),
                                                                       NewModState :: term()}.
-callback handoff_starting(handoff_dest(), ModState :: term()) -> {boolean(),
                                                                   NewModState :: term()}.
-callback handoff_cancelled(ModState :: term()) -> {ok, NewModState :: term()}.
-callback handoff_finished(handoff_dest(), ModState :: term()) -> {ok,
                                                                   NewModState :: term()}.
-callback handle_handoff_command(Request :: term(),
                                 ...
                                 ModState :: term()) -> {reply,
                                                        ...
                                                        {stop,
                                                         Reason :: term(),
                                                         NewModState :: term()}.
-callback handle_handoff_data(binary(), ModState :: term()) -> {reply,
                                                                ok | {error, Reason :: term()},
                                                                NewModState :: term()}.
-callback encode_handoff_item(Key :: term(), Value :: term()) -> corrupted | binary().
-callback is_empty(ModState :: term()) -> {boolean(), NewModState :: term()} |
...
-callback handle_overload_command(Request :: term(),
                                  Sender :: sender(),
                                  Idx :: partition()) -> ok.
-callback handle_overload_info(Request :: term(), Idx :: partition()) -> ok.
...
started(timeout, State = #state{inactivity_timeout = InitialInactivityTimeout}) ->
...
started(wait_for_init,
        _From,
        State = #state{inactivity_timeout = InitialInactivityTimeout}) ->
    case do_init(State) of
      {ok, State2} ->
      ...
      _ ->
          case lists:keyfind(pool, 1, Props) of
            {pool, WorkerModule, PoolSize, WorkerArgs} = PoolConfig ->
                logger:debug("starting worker pool ~p with size of ~p~n", [WorkerModule, PoolSize]),
...
forward_or_vnode_command(Sender,
                         Request,
                         State = #state{forward = Forward, mod = Mod, index = Index}) ->
...
vnode_command(_Sender, _Request, State = #state{modstate = {deleted, _}}) ->
    continue(State);
vnode_command(Sender,
              Request,
              State = #state{mod = Mod, modstate = ModState, pool_pid = Pool}) ->
...
vnode_resize_command(Sender, Request, FutureIndex, State = #state{forward = Forward})
    when is_list(Forward) ->
    case lists:keyfind(FutureIndex, 1, Forward) of
      false ->
          vnode_command(Sender, Request, State);
      {FutureIndex, FutureOwner} ->
          vnode_handoff_command(Sender, Request, {FutureIndex, FutureOwner}, State)
    end.
    ...
    case RequestHash of
      ...
      _ ->
          ...
          case FutureIndex of
            ...
            HOIdx ->
                vnode_handoff_command(Sender, Request, {HOIdx, HONode}, State);
...
active({resize_transfer_complete, SeenIdxs},
       State = #state{mod = Mod, modstate = ModState, handoff_target = Target}) ->
    case Target of
      none ->
          continue(State);
      _ ->
...
active(unregistered, State = #state{mod = Mod, index = Index}) ->
    ...
    logger:debug("~p ~p vnode excluded and unregistered.", [Index, Mod]),
    {stop,
     normal,
     State#state{handoff_target = none, handoff_type = undefined, pool_pid = undefined}}.
...
mark_handoff_complete(SrcIdx, Target, SeenIdxs, Mod, resize) ->
    ...
    Result = riak_core_ring_manager:ring_trans(fun (Ring, _) ->
                                                       Owner = riak_core_ring:index_owner(Ring,
                                                                                          SrcIdx),
                                                       Status =
    ...
    Result = riak_core_ring_manager:ring_trans(fun (Ring, _) ->
                                                       ...
                                                       {_, NextOwner, Status} =
                                                           riak_core_ring:next_owner(Ring,
                                                                                     ...
                                                                                     Mod),
                                                       NewStatus =
                                                           riak_core_ring:member_status(Ring, New),
...
finish_handoff(SeenIdxs,
    ...
    case mark_handoff_complete(Idx, Target, SeenIdxs, Mod, HOType) of
      ...
      Res when Res == forward; Res == shutdown ->
          ...
          maybe_shutdown_pool(State),
          {ok, NewModState} = Mod:delete(ModState),
          logger:debug("~p ~p vnode finished handoff and deleted.", [Idx, Mod]),
          riak_core_vnode_manager:unregister_vnode(Idx, Mod),
...
mark_delete_complete(Idx, Mod) ->
    Result = riak_core_ring_manager:ring_trans(fun (Ring, _) ->
                                                       ...
                                                       {_, Next, Status} =
                                                           riak_core_ring:next_owner(Ring, Idx),
...
handle_event({set_forwarding, undefined},
             _StateName,
...
handle_sync_event(get_mod_index,
                  _From,
                  StateName,
                  State = #state{index = Idx, mod = Mod}) ->
    {reply, {Mod, Idx}, StateName, State, State#state.inactivity_timeout};
handle_sync_event({handoff_data, _BinObj},
                  _From,
                  StateName,
                  State = #state{modstate = {deleted, _ModState}}) ->
    {reply, {error, vnode_exiting}, StateName, State, State#state.inactivity_timeout};
handle_sync_event({handoff_data, BinObj},
                  _From,
                  StateName,
                  State = #state{mod = Mod, modstate = ModState}) ->
...
handle_info({'EXIT', Pid, Reason},
            _StateName,
            State = #state{mod = Mod, index = Index, pool_pid = Pid, pool_config = PoolConfig}) ->
    case Reason of
      ...
      _ ->
          ...
          {pool, WorkerModule, PoolSize, WorkerArgs} = PoolConfig,
          logger:debug("starting worker pool ~p with size of ~p for vnode ~p.",
                       [WorkerModule, PoolSize, Index]),
          {ok, NewPoolPid} = riak_core_vnode_worker_pool:start_link(WorkerModule,
                                                                    PoolSize,
                                                                    Index,
                                                                    WorkerArgs,
                                                                    worker_props),
...
maybe_handoff(TargetIdx,
    ...
    ValidHN = case CurrentTarget of
                ...
                Target ->
                    not ExistingHO;
                _ ->
                    logger:info("~s/~b: handoff request to ~p before finishing handoff to ~p",
