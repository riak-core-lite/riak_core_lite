...
-export([all_members/1,
         ...
         update_meta/3,
         remove_meta/2]).
-export([cluster_name/1, set_tainted/1, check_tainted/2, nearly_equal/2, claimant/1,
         member_status/2, pretty_print/2, all_member_status/1, update_member_meta/5,
         clear_member_meta/3, get_member_meta/3, add_member/3, remove_member/3, leave_member/3,
         exit_member/3, down_member/3, set_member/4, set_member/5, members/2, set_claimant/2,
         increment_vclock/2, ring_version/1, increment_ring_version/2, set_pending_changes/2,
         active_members/1, claiming_members/1, ready_members/1, random_other_active_node/1,
         down_members/1, set_owner/2, indices/2, future_indices/2, future_ring/1,
         disowning_indices/2, cancel_transfers/1, pending_changes/1, next_owner/1, next_owner/2,
         next_owner/3, completed_next_owners/2, all_next_owners/1, change_owners/2,
         handoff_complete/3, ring_ready/0, ring_ready/1, ring_ready_info/1, ring_changed/2,
         set_cluster_name/2, reconcile_names/2, reconcile_members/2, is_primary/2, chash/1,
         set_chash/2, resize/2, set_pending_resize/2, set_pending_resize_abort/1,
         maybe_abort_resize/1, schedule_resize_transfer/3, awaiting_resize_transfer/3,
         resize_transfer_status/4, resize_transfer_complete/4, complete_resize_transfers/3,
         reschedule_resize_transfers/3, is_resizing/1, is_post_resize/1, is_resize_complete/1,
         resized_ring/1, set_resized_ring/2, future_index/3, future_index/4, future_index/5,
         is_future_index/4, future_owner/2, future_num_partitions/1, vnode_type/2,
         deletion_complete/3]).%%         upgrade/1,
                               %%         downgrade/2,
...
-type pending_change() :: {Owner :: node(), NextOwner :: node(), awaiting | complete} |
...
-spec all_preflists(State :: chstate(), N :: integer()) -> [[{Index :: integer(),
                                                              Node :: term()}]].
all_preflists(State, N) ->
    [lists:sublist(preflist(Key, State), N)
     || Key <- [<<(I + 1):160/integer>> || {I, _Owner} <- ?MODULE:all_owners(State)]].
...
-spec get_meta(Key :: term(), State :: chstate()) -> {ok, term()} | undefined.
get_meta(Key, State) ->
...
-spec preflist(Key :: binary(), State :: chstate()) -> [{Index :: chash:index_as_int(),
                                                         Node :: term()}].
preflist(Key, State) ->
...
-spec reconcile(ExternState :: chstate(), MyState :: chstate()) -> {no_change | new_ring,
                                                                    chstate()}.
reconcile(ExternState, MyState) ->
    check_tainted(ExternState,
                  "Error: riak_core_ring/reconcile :: reconciling tainted external "
                  "ring"),
    check_tainted(MyState,
                  "Error: riak_core_ring/reconcile :: reconciling tainted internal "
                  "ring"),
...
-spec rename_node(State :: chstate(), OldNode :: atom(), NewNode :: atom()) -> chstate().
rename_node(State = #chstate{chring = Ring,
    ...
    State#chstate{chring =
                  ...
                  members =
                      orddict:from_list(proplists:substitute_aliases([{OldNode, NewNode}],
                                                                     Members)),
                  seen =
                      orddict:from_list(proplists:substitute_aliases([{OldNode, NewNode}], Seen)),
                  nodename =
                      case ThisNode of
                        OldNode ->
                            NewNode;
...
-spec future_index(chash:index(), integer(), chstate()) -> integer() | undefined.
...
reconcile_names(RingA = #chstate{clustername = NameA},
    ...
    case (NameA =:= undefined) or (NameB =:= undefined) of
      true ->
          {RingA#chstate{clustername = undefined}, RingB#chstate{clustername = undefined}};
      false ->
          {RingA, RingB}
    end.
...
clear_member_meta(Node, State, Member) ->
    ...
    case orddict:is_key(Member, Members) of
      true ->
          Members2 = orddict:update(Member,
                                    fun ({Status, VC, _MD}) ->
                                            {Status, vclock:increment(Node, VC), orddict:new()}
                                    end,
                                    Members),
          State#chstate{members = Members2};
      false ->
...
disowning_indices(State, Node) ->
    case is_resizing(State) of
      false ->
          [Idx || {Idx, Owner, _NextOwner, _Mods, _Status} <- State#chstate.next, Owner =:= Node];
      true ->
          [Idx
           || {Idx, Owner} <- all_owners(State),
              Owner =:= Node,
...
schedule_resize_transfer(State, Source, Target) ->
    ...
    case lists:keymember(Target, 1, Transfers) of
      true ->
          State;
      false ->
          Transfers1 = lists:keystore(Target, 1, Transfers, {Target, ordsets:new(), awaiting}),
...
reschedule_resize_transfers(State = #chstate{next = Next}, Node, NewNode) ->
    {NewNext, NewState} = lists:mapfoldl(fun (Entry, StateAcc) ->
...
reschedule_resize_operation(Node,
                            ...
                            State) ->
    {Changed, NewState} = reschedule_inbound_resize_transfers({Idx, OtherNode},
                                                              Node,
                                                              NewNode,
                                                              State),
    case Changed of
...
-spec awaiting_resize_transfer(chstate(), {integer(), term()}, atom()) -> {integer(),
                                                                           term()} |
                                                                          undefined.
...
resize_transfer_complete(State, {SrcIdx, _} = Source, Target, Mod) ->
    ...
    case Transfer of
      {Target, Mods, Status} ->
          ...
          ResizeTransfers2 = lists:keyreplace(Target, 1, ResizeTransfers, {Target, Mods2, Status2}),
          State1 = set_resize_transfers(State, Source, ResizeTransfers2),
          AllComplete = lists:all(fun ({_, _, complete}) ->
                                          true;
                                      ({_, Ms, awaiting}) ->
...
is_resize_complete(#chstate{next = Next}) ->
    not
      lists:any(fun ({_, _, _, _, awaiting}) ->
                        true;
                    ({_, _, _, _, complete}) ->
                        false
                end,
...
-spec complete_resize_transfers(chstate(), {integer(), term()}, atom()) -> [{integer(),
                                                                             term()}].
complete_resize_transfers(State, Source, Mod) ->
    [Target
     || {Target, Mods, Status} <- resize_transfers(State, Source),
...
ring_ready(State0) ->
    check_tainted(State0, "Error: riak_core_ring/ring_ready called on tainted ring"),
...
ring_ready_info(State0) ->
    ...
    Outdated = orddict:filter(fun (Node, VC) ->
                                      not vclock:equal(VC, RecentVC) and lists:member(Node, Members)
                              end,
                              Seen),
    Outdated.
...
ring_changed(Node, State) ->
    check_tainted(State, "Error: riak_core_ring/ring_changed called on tainted ring"),
...
pretty_print(Ring, Opts) ->
    ...
    case OptLegend of
      true ->
          ...
          _ = [begin
                 NodeIndices = [Idx || {Idx, Owner} <- Indices, Owner =:= Node],
    ...
    case Numeric of
      true ->
          Ownership = [orddict:fetch(Owner, Names) || {_Idx, Owner} <- Indices],
...
log_ring_result(#chstate{vclock = V, members = Members, next = Next}) ->
    logger:debug("Updated ring vclock: ~p, Members: ~p, Next: ~p", [V, Members, Next]).
...
reconcile_divergent(VNode, StateA, StateB) ->
    VClock = vclock:increment(VNode,
                              vclock:merge([StateA#chstate.vclock, StateB#chstate.vclock])),
...
reconcile_members(StateA, StateB) ->
    orddict:merge(fun (_K, {Valid1, VC1, Meta1}, {Valid2, VC2, Meta2}) ->
...
exclusion_test() ->
    ...
    Ring1 = transfer_node(0, x, Ring0),
    ?assertEqual(0,
                 random_other_index(Ring1, [730750818665451459101842416358141509827966271488])),
...
membership_test() ->
    ...
    [begin
     ...
     || {StatusA, PriorityA} <- Priority, {StatusB, PriorityB} <- Priority],
...
ring_version_test() ->
    ...
    {_, RingT1} = reconcile(RingA2#chstate{rvsn = vclock:increment(nodeA, RVsn)}, RingB2),
    ?assertEqual(nodeA, index_owner(RingT1, 0)),

    %% RingB1 has most recent ring version
    {_, RingT2} = reconcile(RingA2, RingB2#chstate{rvsn = vclock:increment(nodeB, RVsn)}),
    ...
    {setup,
     fun () ->
             ...
             meck:expect(riak_core,
                         vnode_modules,
                         fun () ->
                                 [{some_app, fake_vnode}, {other_app, other_vnode}]
                         end)
     end,
     fun (_) ->
             meck:unload()
...
test_resize_xfers() ->
    ...
    ?assertEqual(awaiting,
                 resize_transfer_status(Ring4, Source1, {TargetIdx2, a}, fake_vnode)),
