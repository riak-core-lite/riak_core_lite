...
standard_join(Node, Ring, Rejoin, Auto) ->
    ...
    SameSize = riak_core_ring:num_partitions(MyRing) =:= riak_core_ring:num_partitions(Ring),
    ...
    case {InitComplete, Rejoin or Singleton, SameSize} of
      ...
      _ ->
          ...
          Ring4 = riak_core_ring:update_member_meta(node(), Ring3, node(), gossip_vsn, 2),
          Ring5 = Ring4,
...
standard_remove(Node) ->
    riak_core_ring_manager:ring_trans(fun (Ring2, _) ->
                                              Ring3 = riak_core_ring:remove_member(node(),
                                                                                   Ring2,
                                                                                   Node),
    ...
    case net_adm:ping(Node) of
      ...
      pang ->
          case {riak_core_ring:all_members(Ring), riak_core_ring:member_status(Ring, Node)} of
            ...
            _ ->
                riak_core_ring_manager:ring_trans(fun (Ring2, _) ->
                                                          Ring3 = riak_core_ring:down_member(node(),
                                                                                             Ring2,
                                                                                             Node),
                                                          Ring4 =
...
standard_leave(Node) ->
    riak_core_ring_manager:ring_trans(fun (Ring2, _) ->
                                              Ring3 = riak_core_ring:leave_member(Node,
                                                                                  Ring2,
                                                                                  Node),
                                              {new_ring, Ring3}
                                      end,
...
register(_App, []) ->
    ...
    %% the ring.
    {ok, _R} = riak_core_ring_manager:ring_trans(fun (R, _A) ->
                                                         {new_ring, R}
                                                 end,
                                                 undefined),
...
register_mod(App, Module, Type) when is_atom(Type) ->
    ...
    case application:get_env(riak_core, Type) of
      ...
      {ok, Mods} ->
          application:set_env(riak_core, Type, lists:usort([{App, Module} | Mods]))
    end.
...
register_metadata(App, Value, Type) ->
    case application:get_env(riak_core, Type) of
      ...
      {ok, Values} ->
          application:set_env(riak_core, Type, lists:usort([{App, Value} | Values]))
    end.
...
register_proplist({Key, Value}, Type) ->
    case application:get_env(riak_core, Type) of
      ...
      {ok, Values} ->
          application:set_env(riak_core, Type, lists:keystore(Key, 1, Values, {Key, Value}))
    end.
