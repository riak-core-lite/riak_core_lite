...
-record(state, {mode, raw_ring, ring_changed_time, inactivity_timer}).

-export([setup_ets/1,
         cleanup_ets/1,
         set_ring_global/1,
         promote_ring/0]). %% For EUnit testing
...
reload_ring(live) ->
    case riak_core_ring_manager:find_latest_ringfile() of
      ...
      {error, Reason} ->
          logger:critical("Failed to load ring file: ~p", [riak_core_util:posix_error(Reason)]),
          throw({error, Reason})
    end.
...
handle_call({ring_trans, Fun, Args}, _From, State = #state{raw_ring = Ring}) ->
    case catch Fun(Ring, Args) of
      ...
      Other ->
          logger:error("ring_trans: invalid return value: ~p", [Other]),
          {reply, not_changed, State}
    end;
...
run_fixups([{App, Fixup} | T], BucketName, BucketProps) ->
    BP = try Fixup:fixup(BucketName, BucketProps) of
           {ok, NewBucketProps} ->
               NewBucketProps;
           {error, Reason} ->
               logger:error("Error while running bucket fixup module ~p from application "
                            "~p on bucket ~p: ~p",
                            [Fixup, App, BucketName, Reason]),
               BucketProps
         catch
           What:Why ->
               logger:error("Crash while running bucket fixup module ~p from application "
                            "~p on bucket ~p : ~p:~p",
...
set_ring_global(Ring) ->
    ...
    FixedRing = case riak_core:bucket_fixups() of
                  ...
                  Fixups ->
                      ...
                      lists:foldl(fun (Bucket, AccRing) ->
                                          ...
                                          MergedProps = riak_core_bucket:merge_props(BucketProps,
                                                                                     DefaultProps),
    ...
    Actions = [{ring, TaintedRing}, {raw_ring, Ring}, {id, {Epoch, Id + 1}}, {chashbin, CHBin}
               | BucketMeta2],
