...
exit_ring_trans() ->
    riak_core_ring_manager:ring_trans(fun (Ring2, _) ->
                                              Ring3 = riak_core_ring:exit_member(node(),
                                                                                 Ring2,
                                                                                 node()),
                                              {new_ring, Ring3}
                                      end,
...
ready_to_exit([{_App, Mod} | AppMods]) ->
    case erlang:function_exported(Mod, ready_to_exit, 0) andalso not Mod:ready_to_exit() of
      true ->
          false;
      false ->
...
ensure_vnodes_started({App, Mod}, Ring) ->
    ...
    spawn_link(fun () ->
                       ...
                       RegName = list_to_atom("riak_core_ring_handler_ensure_" ++
                                                atom_to_list(Mod)),
                       try
                       ...
                       %% Start the vnodes.
                       HasStartVnodes = lists:member({start_vnodes, 1}, Mod:module_info(exports)),
