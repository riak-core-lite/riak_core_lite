...
-spec partition_id_to_hash(riak_core_ring:partition_id(),
...
boundary_test() ->
    ...
    ?assertNot(riak_core_ring_util:hash_is_partition_boundary(<<(BoundaryIndex + 32):160>>,
                                                              32)),
    ?assertNot(riak_core_ring_util:hash_is_partition_boundary(<<(BoundaryIndex - 32):160>>,
                                                              32)),
    ?assertNot(riak_core_ring_util:hash_is_partition_boundary(<<(BoundaryIndex + 1):160>>,
                                                              32)),
    ?assertNot(riak_core_ring_util:hash_is_partition_boundary(<<(BoundaryIndex - 1):160>>,
                                                              32)),
    ?assertNot(riak_core_ring_util:hash_is_partition_boundary(<<(BoundaryIndex + 2):160>>,
                                                              32)),
    ?assertNot(riak_core_ring_util:hash_is_partition_boundary(<<(BoundaryIndex + 10):160>>,
                                                              32)).
...
prop_ids_are_boundaries() ->
    ?FORALL(RingPower,
            choose(2, ?RINGSIZEEXPMAX),
            ?FORALL(PartitionId,
                    choose(0, ?RINGSIZE(RingPower) - 1),
                    begin
                      RingSize = ?RINGSIZE(RingPower),
                      BoundaryHash = riak_core_ring_util:partition_id_to_hash(PartitionId,
                                                                              RingSize),
                      equals(true,
                             riak_core_ring_util:hash_is_partition_boundary(BoundaryHash, RingSize))
                    end)).
            ...
            ?FORALL(PartitionId,
                    ...
                    begin
                      ...
                      BoundaryHash = riak_core_ring_util:partition_id_to_hash(PartitionId,
                                                                              RingSize),
                      equals(PartitionId,
                             riak_core_ring_util:hash_to_partition_id(BoundaryHash, RingSize))
                    end)).
...
prop_monotonic() ->
    ?FORALL(RingPower,
            ...
            ?FORALL(HashValue,
                    choose(0, ?HASHMAX - 1),
                    ?FORALL(GreaterHash,
                            choose(HashValue + 1, ?HASHMAX),
                            begin
                              RingSize = ?RINGSIZE(RingPower),
                              LowerPartition = riak_core_ring_util:hash_to_partition_id(HashValue,
                                                                                        RingSize),
                              GreaterPartition =
                                  riak_core_ring_util:hash_to_partition_id(GreaterHash, RingSize),
...
ring_to_set({_RingSize, PropList}) ->
    ordsets:from_list(lists:map(fun ({Hash, dummy}) ->
                                        Hash
                                end,
                                PropList)).
...
find_near_boundaries(RingSize, PartitionSize) ->
    ?LET({Id, Offset},
         {choose(1, RingSize - 1), choose(-(RingSize * 2), RingSize * 2)},
            ...
            ?FORALL({HashValue, BoundarySet},
                    ...
                    begin
                      ...
                      HashIsPartitionBoundary =
                          riak_core_ring_util:hash_is_partition_boundary(HashValue, RingSize),
