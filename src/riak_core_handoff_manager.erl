...
-record(state, {excl, handoffs = [] :: [handoff_status()]}).

%% this can be overridden with riak_core handoff_concurrency
-define(HANDOFF_CONCURRENCY, 2).
-define(HO_EQ(HOA, HOB),
        HOA#handoff_status.mod_src_tgt == HOB#handoff_status.mod_src_tgt andalso
          HOA#handoff_status.timestamp == HOB#handoff_status.timestamp).
...
handle_call({add_outbound, Type, Mod, SrcIdx, TargetIdx, Node, Pid, Opts},
            _From,
...
handle_call({set_recv_data, Recv, Data}, _From, State = #state{handoffs = HS}) ->
    case lists:keyfind(Recv, #handoff_status.transport_pid, HS) of
      false ->
          throw({error, "set_recv_data called for non-existing receiver", Recv, Data});
      #handoff_status{} = H ->
          H2 = H#handoff_status{mod_src_tgt = proplists:get_value(mod_src_tgt, Data),
...
handle_call({kill_in_direction, Direction}, _From, State = #state{handoffs = HS}) ->
    ...
    _ = [erlang:exit(Pid, max_concurrency) || #handoff_status{transport_pid = Pid} <- Kill],
    {reply, ok, State}.
          ...
          case lists:keytake(Ref, #handoff_status.vnode_mon, HS) of
            {value,
             ...
             NewHS} ->
                ...
                logger:error("An ~w handoff of partition ~w ~w was terminated because the "
                             "vnode died",
                             [Dir, M, I]),
                demonitor(TransM),
...
resize_transfer_filter(Ring, Mod, Src, Target) ->
    fun (K) ->
            ...
            riak_core_ring:is_future_index(Hashed, Src, Target, Ring)
    end.
...
resize_transfer_notsent_fun(Ring, Mod, Src) ->
    ...
    case Shrinking of
      false ->
          NValMap = DefaultN = undefined;
      true ->
          NValMap = Mod:nval_map(Ring),
          DefaultN = riak_core_bucket:default_object_nval()
    end,
    fun (Key, Acc) ->
            record_seen_index(Ring, Shrinking, NValMap, DefaultN, Mod, Src, Key, Acc)
    end.
...
record_seen_index(Ring, Shrinking, NValMap, DefaultN, Mod, Src, Key, Seen) ->
    ...
    CheckNVal = case Shrinking of
                  false ->
                      undefined;
                  true ->
                      proplists:get_value(Bucket, NValMap, DefaultN)
                end,
...
-spec send_handoff(ho_type(),
                   ...
                   pid(),
                   list(),
                   {predicate() | none, {module(), atom()} | none},
                   node(),
...
send_handoff(HOType,
    ...
    case handoff_concurrency_limit_reached() of
      true ->
          {error, max_concurrency};
      false ->
          ShouldHandoff = case lists:keyfind({Mod, Src, Target}, #handoff_status.mod_src_tgt, HS) of
                            false ->
                                true;
          ...
          case ShouldHandoff of
            true ->
                ...
                HOOpts = [{filter, HOFilter}, {notsent_acc0, HOAcc0}, {notsent_fun, HONotSentFun}
                          | BaseOpts],
...
validate_size(Size = {N, U})
    when is_number(N) andalso N > 0 andalso (U =:= bytes orelse U =:= objects) ->
    Size;
    ...
    case lists:keytake(ModSrcTarget, #handoff_status.mod_src_tgt, HS) of
      ...
      {value, Xfer, HS2} ->
          ...
          case Type of
            ...
            _ ->
                logger:info(Msg,
                            [Type, Mod, SrcNode, SrcPartition, TargetNode, TargetPartition, Reason])
          end,
...
handoff_change_enabled_setting(EnOrDis, Direction) ->
    SetFun = case EnOrDis of
               enable ->
                   fun handoff_enable/1;
               disable ->
                   fun handoff_disable/1
             end,
    case Direction of
      inbound ->
          SetFun(inbound);
...
handoff_test_() ->
    {spawn,
     {setup,
      ...
      %% actual list of test
      [?_test(simple_handoff()), ?_test(config_disable())]}}.
