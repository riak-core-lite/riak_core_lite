...
-export([start_link/0, set_socket/2, supports_batching/0]).
-export([init/1,
         handle_call/3,
         handle_cast/2,
         handle_info/2,
...
init([]) ->
    {ok,
     #state{recv_timeout_len =
                application:get_env(riak_core, handoff_receive_timeout, ?RECV_TIMEOUT),
            vnode_timeout_len =
                application:get_env(riak_core, handoff_receive_vnode_timeout, ?VNODE_TIMEOUT)}}.
...
handle_info({tcp, Socket, Data}, State) ->
    ...
    case catch process_message(MsgType, MsgData, State) of
      {'EXIT', Reason} ->
          logger:error("Handoff receiver for partition ~p exited abnormally after processing "
                       "~p objects from ~p: ~p",
...
handle_info(timeout, State) ->
    logger:error("Handoff receiver for partition ~p timed out after processing "
                 "~p objects from ~p.",
                 [State#state.partition, State#state.count, State#state.peer]),
    {stop, normal, State}.

process_message(?PT_MSG_INIT,
                MsgData,
                State = #state{vnode_mod = VNodeMod, peer = Peer}) ->
    <<Partition:160/integer>> = MsgData,
    logger:info("Receiving handoff data for partition ~p:~p from ~p",
                [VNodeMod, Partition, Peer]),
    {ok, VNode} = riak_core_vnode_master:get_vnode_pid(Partition, VNodeMod),
    Data = [{mod_src_tgt, {VNodeMod, undefined, Partition}}, {vnode_pid, VNode}],
...
process_message(?PT_MSG_OBJ,
                MsgData,
                State = #state{vnode = VNode, count = Count, vnode_timeout_len = VNodeTimeout}) ->
...
process_message(?PT_MSG_SYNC, _MsgData, State = #state{sock = Socket}) ->
    ...
    State;
process_message(?PT_MSG_VERIFY_NODE,
                ExpectedName,
    ...
    case binary_to_term(ExpectedName) of
      _Node when _Node =:= node() ->
          ...
          State;
      Node ->
          logger:error("Handoff from ~p expects us to be ~s but we are ~s.", [Peer, Node, node()]),
