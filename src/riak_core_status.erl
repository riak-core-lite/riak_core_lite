...
-export([ringready/0, all_active_transfers/0, transfers/0, partitions/2, ring_status/0]).
...
-spec transfers() -> {[atom()],
                      [{waiting_to_handoff, atom(), integer()} | {stopped, atom(), integer()}]}.
transfers() ->
...
all_active_transfers() ->
    {Xfers, Down} = riak_core_util:rpc_every_member(riak_core_handoff_manager,
                                                    status,
                                                    [{direction, outbound}],
                                                    5000),
    {Xfers, Down}.
...
ring_status() ->
    ...
    {AllMods, Down} = riak_core_util:rpc_every_member_ann(riak_core, vnode_modules, [], 1000),
    ...
    Merged = lists:foldl(fun ({Idx, Owner, NextOwner, Mods, Status}, Acc) ->
                                 orddict:append({Owner, NextOwner}, {Idx, Mods, Status}, Acc)
                         end,
                         [],
                         Changes),
    ...
    TransferStatus = orddict:map(fun ({Owner, _}, Transfers) ->
...
get_rings() ->
    {RawRings, Down} = riak_core_util:rpc_every_member(riak_core_ring_manager,
                                                       get_my_ring,
                                                       [],
                                                       30000),
