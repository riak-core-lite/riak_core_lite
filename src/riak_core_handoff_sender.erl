...
-define(LOG_INFO(Str, Args),
        logger:info("~p transfer of ~p from ~p ~p to ~p ~p failed " ++ Str,
                    [Type, Module, SrcNode, SrcPartition, TargetNode, TargetPartition] ++ Args)).
...
start_fold_(TargetNode,
    ...
    case gen_tcp:recv(Socket, 0, RecvTimeout) of
      {ok, [?PT_MSG_VERIFY_NODE | _]} ->
          ok;
      {ok, [?PT_MSG_UNKNOWN | _]} ->
          logger:warning("Could not verify identity of peer ~s.", [TargetNode]),
          ok;
      {error, timeout} ->
          exit({shutdown, timeout});
      {error, closed} ->
          exit({shutdown, wrong_node})
    end,
    ...
    AccRecord0 = case riak_core_vnode_master:sync_command({SrcPartition, SrcNode},
                                                          Req,
                                                          VMaster,
                                                          infinity)
                     of
    ...
    case ErrStatus of
      ok ->
          ...
          logger:debug("~p ~p Sending final sync", [SrcPartition, Module]),
          ok = gen_tcp:send(Socket, <<?PT_MSG_SYNC:8>>),

          case gen_tcp:recv(Socket, 0, RecvTimeout) of
            {ok, [?PT_MSG_SYNC | <<"sync">>]} ->
                logger:debug("~p ~p Final sync received", [SrcPartition, Module]);
            {error, timeout} ->
                exit({shutdown, timeout})
          end,
          ...
          ok = logger:info("~p transfer of ~p from ~p ~p to ~p ~p completed: sent ~p bytes "
                           "in ~p of ~p objects in ~p seconds (~p/second)",
                           [Type,
                            Module,
                            SrcNode,
                            SrcPartition,
          ...
          case Type of
            repair ->
                ok;
            resize ->
                gen_fsm_compat:send_event(ParentPid, {resize_transfer_complete, NotSentAcc});
            _ ->
...
start_fold(TargetNode, Module, {Type, Opts}, ParentPid) ->
    ...
    try
      ...
      exit:{shutdown, {error, Reason}} ->
          ...
          gen_fsm_compat:send_event(ParentPid, {handoff_error, fold_error, Reason}),
          exit({shutdown, {error, Reason}});
      {be_quiet, Err, Reason} ->
...
visit_item2(K,
            V,
            Acc = #ho_acc{ack = _AccSyncThreshold, acksync_threshold = _AccSyncThreshold}) ->
    ...
    case Filter(K) of
      true ->
          case Module:encode_handoff_item(K, V) of
            corrupted ->
                {Bucket, Key} = K,
                logger:warning("Unreadable object ~p/~p discarded", [Bucket, Key]),
                Acc;
            BinObj ->
                case UseBatching of
                  true ->
      ...
      false ->
          ...
          Acc#ho_acc{error = ok, total_objects = TotalObjects + 1, notsent_acc = NewNotSentAcc}
    end.
...
remote_supports_batching(Node) ->
    case catch rpc:call(Node, riak_core_handoff_receiver, supports_batching, []) of
      true ->
