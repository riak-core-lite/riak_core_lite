...
-record(state, {module :: atom(), modstate :: mod_state()}).

-callback init_worker(partition(), Args :: term(), Props :: [{atom(), term()}]) -> {ok,
                                                                                    mod_state()}.
-callback handle_work(Work :: term(), sender(), mod_state()) -> {reply,
                                                                 Reply :: term(),
                                                                 mod_state()} |
                                                                {noreply, mod_state()}.
...
start_link(Args) ->
    ...
    gen_server:start_link(?MODULE,
                          [WorkerMod, VNodeIndex, WorkerArgs, WorkerProps, Caller],
                          []).
